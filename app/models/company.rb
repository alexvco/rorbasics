class Company < ApplicationRecord
  ...
  # During bulk insertion, violation of primary key and other database level validations is possible (ie: not null, unique)
  # ActiveRecord Callbacks or Validations will not be invoked, hence in this ^^ example, validation of format will not be checked (nor will presence: true since not at db level) when doing a bulk insert
  ...
  # ON CONFLICT DO NOTHING RETURNING "a", "b", "c" - this part is perhaps not required and is postgresql/sqlite syntax only,
  # the default RETURNING is the ID for postgresql/sqlite
  # equivalent syntax for mysql: ON DUPLICATE KEY UPDATE 'id'='id', Note MySQL does not support RETURING clause, and therefore, the result doesn’t contain any useful information.
